...
router.post('/:id/verify', async (req, res, next) => {
    try {
        ...
        if (link.file) {
            ...
            const url = await getPresignedUrl(link.file.storedName, 60 * 60, {
                'response-content-disposition': `attachment; filename="${link.file.originalName}"`
            });
            return res.json({ downloadUrl: url, type: 'file' });
        }
