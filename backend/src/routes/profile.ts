...
router.get('/', protect, async (req: AuthRequest, res, next) => {
    try {
        ...
        // If avatar is a file ID, get the file URL
        let avatarUrl = null;
        if (user.avatar) {
            try {
...
router.post('/avatar', protect, async (req: AuthRequest, res, next) => {
    ...
    bb.on('file', async (name: string, file: any, info: any) => {
        ...
        try {
            const storedName = `avatar-${uuidv4()}-${filename}`;
            ...
            res.json({
                ...updated,
                avatarUrl: `${process.env.API_URL || process.env.FRONTEND_URL?.replace('cloud.', 'api.') || 'http://localhost:5000'}/api/files/${newFile.id}/preview`,
            });
        } catch (err) {
