...
export function FileBrowser({ refreshTrigger, searchQuery = "" }: FileBrowserProps) {
    ...
    const handleDownload = async (id: string) => {
        try {
            const res = await axios.get(`${API}/api/files/${id}/download`, { withCredentials: true });
    ...
    return (
        ...
        >
                                ...
                                {filteredFiles.map(file => (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center justify-end gap-0.5 sm:gap-1.5 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                                            ...
                                            <button onClick={(e) => { e.stopPropagation(); confirmCreateLink(file.id); }} className="p-2 sm:p-2.5 glass-subtle rounded-xl text-primary hover:bg-muted transition-all shrink-0" title="Share">
                                                <LinkIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                                            </button>
                                            <button onClick={(e) => { e.stopPropagation(); confirmDelete(file.id, file.originalName, 'file'); }} className="p-2 sm:p-2.5 text-muted-foreground hover:text-red-500 transition-colors shrink-0" title="Delete">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6 selection-zone">
                                    ...
                                    >
                                        ...
                                        <div
                                            ...
                                            className={cn(
                                                ...
                                                selectedItems.find(i => i.id === folder.id)
                                                    ? "bg-primary border-primary text-white"
                                                    : "bg-white/80 border-white group-hover:border-primary/40 opacity-0 group-hover:opacity-100"
                                            )}
                                        >
                                            {selectedItems.find(i => i.id === folder.id) && <CheckCircle className="w-4 h-4" />}
                                        </div>
                                        <div className="w-full aspect-square rounded-[1.25rem] flex items-center justify-center relative overflow-hidden bg-primary/[0.03] group-hover:bg-primary/[0.08] transition-colors">
                                            <FolderIcon className="w-16 h-16 text-primary/30 fill-primary/10 transition-transform group-hover:scale-110 duration-500" />

                                            <div className="absolute inset-x-2 bottom-2 flex items-center justify-center gap-1.5 opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0">
                                                <button onClick={(e) => { e.stopPropagation(); navigateToFolder(folder); }} className="p-2.5 glass-subtle rounded-xl shadow-sm hover:bg-background text-primary transition-colors" title="Open">
                                                    <ChevronRight className="w-5 h-5" />
                                                </button>
                                                <button onClick={(e) => { e.stopPropagation(); confirmDelete(folder.id, folder.name, 'folder'); }} className="p-2.5 glass-subtle rounded-xl shadow-sm hover:bg-red-50 text-red-500 transition-colors" title="Delete">
                                                    <Trash2 className="w-4 h-4" />
                                                </button>
                                            </div>
                                        </div>
                                ...
                                {filteredFiles.map(file => (
                                    <motion.div
                                        ...
                                        onDrop={(e: any) => handleDrop(e, null)} // Drop on items doesn't make sense for files usually but added for type safety if needed
                                        whileDrag={{ scale: 1.05, opacity: 0.8, rotate: -2 }}
                                        className={cn(
                                    ...
                                    >
                                        ...
                                        <div className="w-full aspect-square rounded-[1.25rem] flex items-center justify-center relative overflow-hidden bg-muted/40 group-hover:bg-muted/60 transition-colors">
                                            {previews[file.id] ? (
                                            ...
                                            <div className="absolute inset-x-2 bottom-2 flex items-center justify-center gap-1.5 opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0">
                                                <button onClick={(e) => { e.stopPropagation(); handleDownload(file.id); }} className="p-2.5 glass-subtle rounded-xl shadow-sm hover:bg-background text-foreground transition-colors" title="Download">
                                                    <Download className="w-4 h-4" />
                                                </button>
                                                <button onClick={(e) => { e.stopPropagation(); confirmCreateLink(file.id); }} className="p-2.5 glass-subtle rounded-xl shadow-sm hover:bg-background text-primary transition-colors" title="Share">
                                                    <LinkIcon className="w-4 h-4" />
                                                </button>
                                                <button onClick={(e) => { e.stopPropagation(); confirmDelete(file.id, file.originalName, 'file'); }} className="p-2.5 glass-subtle rounded-xl shadow-sm hover:bg-red-50 text-red-500 transition-colors" title="Delete">
                                                    <Trash2 className="w-4 h-4" />
                                                </button>
                                            </div>
                                        </div>
            ...
            <UploadProgress hasSelection={selectedItems.length > 0} />
        </div >
